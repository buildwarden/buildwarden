...
// ScriptResult holds the translated script and the base image
...
// DockerfileToScript translates a Dockerfile/Containerfile into a
// shell script suitable for execution inside a build container.
...
func (b *scriptBuilder) handleDirective(
	...
	case "FROM":
		if b.image != "" {
			return fmt.Errorf(
				"multi-stage builds not yet supported " +
					"(second FROM)")
		}
...
func (b *scriptBuilder) handleCopy(rest string) error {
	if strings.Contains(rest, "--from=") {
		return fmt.Errorf(
			"COPY --from= (multi-stage) not supported")
	}
...
func rejectOrIgnore(directive, rest string) error {
	...
	case "USER":
		return fmt.Errorf(
			"USER directive not supported in script mode "+
				"(build runs as container default user): "+
				"%s", rest)
	case "SHELL":
		return fmt.Errorf(
	...
	case "ADD":
		return fmt.Errorf(
			"ADD not supported " +
				"(use COPY for local files, RUN for remote)")
...
func expandEnv(
	s string, env map[string]string,
) string {
