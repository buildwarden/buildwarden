...
func (e *ExtBazel) BeforeBuild(ctx *ExtensionContext) error {
	bazelrc := `startup --host_jvm_args=-Djavax.net.ssl.trustStore=/.warden/certs.jks \
--host_jvm_args=-Djavax.net.ssl.trustStorePassword=changeit
`
	err := os.WriteFile(filepath.Join(ctx.WardenDir, "bazel.bazelrc"),
		[]byte(bazelrc), 0644)
	if err != nil {
	...
	f, err := os.Create(filepath.Join(ctx.WardenDir, "certs.jks"))
	if err != nil {
...
`
	err = os.WriteFile(filepath.Join(ctx.ScriptDir, "bazel.sh"),
		[]byte(script), 0755)
	if err != nil {
